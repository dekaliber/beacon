...
export function SecuritiesPage() {
 ...
 <span className="rounded bg-warn-soft border border-warn-line px-1.5 py-0.5 text-xs font-medium text-warn-deep whitespace-nowrap">
 28% max
 </span>
 )}
 {!isComplete && (
 ...
 <h2 className="tp-page-title">Securities</h2>
 </div>
 </div>
...
function EditModal({ open, instrument, allInstruments, assetClasses, onClose, onSave }: EditModalProps) {
 ...
 <label className="flex-1 truncate">{ac.name}</label>
 ...
 {/* ── Asset Class Weights ── */}
 <div>
 <h3 className="font-semibold mb-3">Asset Class Weights</h3>
 ...
 <div className={cn(
"flex items-center justify-between rounded-md px-3 py-2 font-medium",
 total > 100.01
 ...
 {/* ── Tax Treatment ── */}
 <div>
 <h3 className="font-semibold mb-2">Tax Treatment</h3>
 ...
 {/* ── Equivalents ── */}
 {!instrument?.isManual && <div>
 <h3 className="font-semibold mb-1">Equivalents</h3>
 ...
 {activeAliases.map((t) => (
 <span key={t.id} className="flex items-center gap-0.5 rounded bg-border px-2 py-0.5 font-medium">
 {t.ticker}
 <button
...
function WeightInput({ id, value, onChange }: { id: string; value: string; onChange: (v: string) => void }) {
 ...
 placeholder="0"
 className="w-full rounded border border-border px-2 py-1 pr-6 text-right focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
 />
...
function InstrumentSelect({
 ...
 className="flex-1 rounded border border-primary px-2 py-1 focus:outline-none focus:ring-1 focus:ring-primary"
 />
 ) : (
 <button
 onClick={startEditing}
 className="flex-1 rounded border border-border px-2 py-1 text-left font-medium hover:border-primary transition-colors"
 >
