...
export function IncomePage() {
 ...
 <span className="select-none cursor-pointer" onClick={() => setStaged((s) => ({ ...s, showOnlyReceived: !s.showOnlyReceived }))}>
...
function IncomeModal({ open, onClose, onSave, onDelete, income, accounts, categories }: IncomeModalProps) {
 ...
 <button type="button" tabIndex={-1} onClick={() => setShowOptional(!showOptional)} className="flex items-center gap-1.5 font-medium text-muted-foreground hover:text-foreground">
 ...
 <textarea name="notes" rows={2} defaultValue={income?.notes ??""} className="w-full rounded-md border border-border px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Additional notes..." />
 </div>
 </div>
 )}
 </div>
