...
function CurrencyInput({ name, defaultValue, required, onChange, autoFocus, error }: { name: string; defaultValue?: string; required?: boolean; onChange?: (value: number) => void; autoFocus?: boolean; error?: boolean }) {
 ...
 onBlur={handleBlur}
 autoFocus={autoFocus}
 className={`w-full rounded-md border pl-7 pr-3 py-2 focus:outline-none focus:ring-1 ${error ?"border-down focus:border-down focus:ring-down/30" :"border-border focus:border-primary focus:ring-primary"}`}
 placeholder="0.00"
 inputMode="text"
 />
 </div>
...
function EditableCategoryCell({
 ...
 key={o.id}
 type="button"
 className={`block w-full px-3 py-1.5 text-left ${i === focusIdx ?"bg-primary/10" :"hover:bg-muted"}`}
...
function EditableAmountCell({ value, onSave, isOffset }: { value: string; onSave: (v: string) => void; isOffset?: boolean }) {
 ...
 return (
 <div className="relative">
 <span className="pointer-events-none absolute left-1 top-1/2 -translate-y-1/2 font-semibold">$</span>
 <input
 ref={inputRef}
 type="text"
 value={editValue}
 ...
 className="w-full rounded border border-primary pl-4 pr-1 py-0.5 text-right font-semibold focus:outline-none"
 inputMode="decimal"
 />
 </div>
 );
...
export function Expenses() {
 ...
 className="appearance-none rounded-md border border-border py-2 pl-2 pr-6 text-foreground"
 >
 ...
 <Card>
 <SectionLabel as="h3" className="mb-3">Upcoming</SectionLabel>
 {(() => {
 ...
 className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full bg-primary px-4 py-2.5 font-medium text-primary-foreground shadow-lg transition-opacity hover:opacity-90"
 ...
 className="flex items-center gap-2 rounded border border-border bg-white px-3 py-1.5 shadow-lg opacity-95"
 >
...
function ExpenseRowWithOffsets({
 ...
 <td
 colSpan={9}
 className={`py-2 px-4 text-center border-2 border-dashed rounded transition-colors ${
 dragState?.targetId ===`__dz__${expense.id}`
...
function ExpenseModal({ open, onClose, onSave, onDelete, onRecurringDelete, expense, offsetParent, categories, accounts, tags, vendors, onCreateTag }: ExpenseModalProps) {
 ...
 className={`w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-1 ${errors.description ?"border-down focus:border-down focus:ring-down/30" :"border-border focus:border-primary focus:ring-primary"}`}
 ...
 placeholder="1"
 className="w-14 rounded-md border border-border px-2 py-1 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
 />
 <div className="relative">
 <select
 name="frequency"
 ...
 <p className="font-medium">Update Recurring Expense</p>
 <p className="text-muted-foreground">
 Would you like to update only this instance, or all future pending instances?
 </p>
